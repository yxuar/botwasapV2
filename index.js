		...
		try {
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                case 'beritahoax':
					...
					reply('Uwuu pp baluu:v, mksi profil nya syng😗')
					break 
					case 'brainly':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
          ...
          if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'clearall':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `perintah Diterima, memblokir beban dunia ${body.slice(7)}@c.us`, text)
					break
                    case 'unblock':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh nomer sayang ku, eh owner mksdnya >_<  jangan spam atau ku block kamu',MessageType.text, { quoted: mek} )
					break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += ` Mangkanya jadi orang jagan baperan,𝗼𝘁𝘄 𝗸𝗶𝗰𝗸 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Mangkanya jadi orang jangan baperan,𝗼𝘁𝘄 𝗸𝗶𝗰𝗸 @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				if (!isQuotedSticker) return reply('𝗥𝗲𝗽𝗹𝘆/𝘁𝗮𝗴 𝘀𝘁𝗶𝗰𝗸𝗲𝗿 !')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: ' *Nohh, bilang apa coba ama bot:v* '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
                ...
                if (args[0] === 'enable') {
                 ...
                 case 'block':
					...
					client.sendMessage(from, `perintah Diterima, memblokir beban dunia wa.me${body.slice(8)}@c.us`, text)
					break
				case 'unblock':
					client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, `perintah Diterima, membuka blokir, kalo ga di buka ntr nagis:v wa.me/${body.slice(10)}`, text)
					break
				case 'welcome':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
